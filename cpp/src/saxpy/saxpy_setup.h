...
struct saxpy_memory {
  ...
  std::unique_ptr<rmm::device_uvector<float>> x;
  std::unique_ptr<rmm::device_uvector<float>> y;

  saxpy_memory(rmm::cuda_stream_view stream) {
    constexpr std::size_t array_size = 1 << 27;
    constexpr std::size_t bytes = array_size * sizeof(float);
    constexpr auto pool_init_size{bytes * 2};
    this->mr =
    ...
    this->x =
        std::make_unique<rmm::device_uvector<float>>(array_size, stream, mr.get());
    this->y =
        std::make_unique<rmm::device_uvector<float>>(array_size, stream, mr.get());
